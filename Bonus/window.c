...
/*   Updated: 2025/02/11 18:50:44 by claghrab         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void cleanup(t_data *data)
{
	...
	if (data->img.player2)
		mlx_destroy_image(data->win.mlx_ptr, data->img.player2);
	if (data->img.colle)
...
void render_tile(t_data *data, int row, int col)
{
	...
	else if (data->game.map[row][col] == 'P')
	{
		...
		img = data->img.door2;
	else if (data->game.map[row][col] == 'N')
	{
		if (f % 1200 < 600)
			img = data->img.player2;
		else
...
void	put_moves(t_data *data)
{
	...
	data->game.x_P * 64 + 28, data->game.y_P * 64 + 40, BLACK, ft_itoa_buff(buf, data->move_count));
}
