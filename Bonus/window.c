...
/*   Updated: 2025/02/15 20:19:49 by claghrab         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	mlx_loop_hook(data->win.mlx_ptr, render_map, data);
...
void	load_images(t_data *data)
{
	...
	data->img.player1 = mlx_xpm_file_to_image(data->win.mlx_ptr,
			...
			&data->img.w,
			&data->img.h);
	data->img.player2 = mlx_xpm_file_to_image(data->win.mlx_ptr,
			"imgs/character2.xpm",
			&data->img.w,
			&data->img.h);
	...
	data->img.door2 = mlx_xpm_file_to_image(data->win.mlx_ptr, "imgs/door2.xpm",
			&data->img.w, &data->img.h);
	data->img.en1 = mlx_xpm_file_to_image(data->win.mlx_ptr, "imgs/enemy1.xpm",
			&data->img.w, &data->img.h);
}
...
void	put_moves(t_data *data)
{
	...
	ft_memset(buf, 0, 127);
	mlx_string_put(data->win.mlx_ptr, data->win.win_ptr, data->game.x_p * 64
		+ 28, data->game.y_p * 64 + 40, BLACK, ft_itoa_buff(buf,
			data->move_count));
}
